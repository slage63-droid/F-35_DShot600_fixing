...
//=========================Implemented changes to Arduino/Teensy Flight Controller - dRehmFlight===========================================
//              swich from OneShot125 to DShot600
//              Changed  output motor pins: M1->pin 11, M2->pin 12, M3->pin 14.
//
//           Adjustment needed: line 1081-1097 -done
//
// 
//
//
...
//========================================================================================================================//
//                                          DSHOT600 CONFIGURATION                                                        //
//========================================================================================================================//
// DShot600: 600 kbps digital motor protocol, used here in unidirectional mode (telemetry disabled).
// Bit-bang implementation using Teensy 4.0 digitalWriteFast() and delayNanoseconds().
// Frame = 11-bit throttle + 1-bit telemetry request + 4-bit CRC (16 bits total, MSB first).
// Motor assignments: M1->pin 11, M2->pin 12, M3->pin 14.

#define DSHOT_MOTOR_COUNT     3     // Number of DShot-controlled motors (M1/M2/M3)
#define DSHOT_MIN_THROTTLE   48     // Lowest DShot throttle value that spins a motor (1-47 reserved)
#define DSHOT_MAX_THROTTLE 2000     // Maximum DShot throttle sent by this controller

// DShot600 bit timing in nanoseconds (bit period = 1/(600,000 bit/s) ≈ 1667 ns)
#define DSHOT600_T1H_NS  1250       // High-pulse width for a logic '1' bit
#define DSHOT600_T1L_NS   417       // Low-pulse width for a logic '1' bit
#define DSHOT600_T0H_NS   625       // High-pulse width for a logic '0' bit
#define DSHOT600_T0L_NS  1042       // Low-pulse width for a logic '0' bit
...
//DShot600 motor pin outputs (M1-M3):
const int m1Pin = 11; //M1 - nose propeller
const int m2Pin = 12; //M2 - right propeller
const int m3Pin = 14; //M3 - left propeller
const int m4Pin = 2;  //unused motor slot
const int m5Pin = 3;  //unused motor slot
const int m6Pin = 5;  //unused motor slot
...
const int servo6Pin = 0;  //relocated from pin 11 (now used by DShot M1)
const int servo7Pin = 1;  //relocated from pin 12 (now used by DShot M2)
...
void setup() {
  ...
  //Arm DShot motors - send DShot stop frames (value 0) for ~300 ms to allow ESCs to arm
  m1_command_PWM = 0;
  m2_command_PWM = 0;
  m3_command_PWM = 0;
  m4_command_PWM = 0;
  m5_command_PWM = 0;
  m6_command_PWM = 0;
  for (int i = 0; i < 300; i++) { //send stop frames at ~1 kHz
    commandMotors();
    delay(1);
  }
...
void loop() {
  ...
  scaleCommands(); //scales motor commands to DShot600 throttle range (0-2000) and servo PWM commands to 0 to 180 (for servo library)

  //Throttle cut check
  throttleCut(); //directly sets motor commands to low based on state of ch5

  //Command actuators
  commandMotors(); //sends DShot600 frames to motor pins 11/12/14 (M1/M2/M3)
...
void controlMixer() {
  ...
  float left_ail_trim_hover = 1.0; //adjust in 0.1 incriments to bring left aileron to ~70 degrees down when in hover mode          0.94+
  float right_ail_trim_hover = 0.0; //adjust in 0.1 incriments to bring right aileron to ~70 degrees down when in hover mode        0.0+
  
  float left_ail_trim_trans = 0.8; //adjust in 0.1 incriments to bring left aileron to ~30 degrees down when in transition mode     0.7+
  float right_ail_trim_trans = 0.2; //adjust in 0.1 incriments to bring right aileron to ~30 degrees down when in transition mode   0.3+
  
  float left_ail_trim_ff = 0.25; //adjust in 0.1 incriments to bring left aileron to level when in forward flight mode              0.25+
  float right_ail_trim_ff = 0.55; //adjust in 0.1 incriments to bring right aileron to level when in forward flight mode             0.7+

  float elevon_gain = 0.5; //adjust to increase/decrease throw of elevon servos in forward flight                                   0.5+
  float aileron_gain = 0.7; //adjust to increase/decrease throw of aileron servos in forward flight                                 0.7+
...
void scaleCommands() {
  ...
  /*
   * mX_command_scaled variables from the mixer function are scaled to the DShot600 throttle range.
   * A scaled value of 0.0 maps to DShot 0 (motor stop). Any positive value maps linearly into
   * DSHOT_MIN_THROTTLE..DSHOT_MAX_THROTTLE so that the motors always spin within a valid range.
   * sX_command_scaled variables are scaled to 0-180 for the servo library using standard PWM.
   * mX_command_PWM are updated here which are used in commandMotors(). sX_command_PWM are used for servos.
   */
  //Scale 0.0-1.0 to DShot600 throttle range (0 = stop, DSHOT_MIN_THROTTLE-DSHOT_MAX_THROTTLE = spinning)
  m1_command_PWM = scaleToDShot(m1_command_scaled);
  m2_command_PWM = scaleToDShot(m2_command_scaled);
  m3_command_PWM = scaleToDShot(m3_command_scaled);
  m4_command_PWM = 0; //unused
  m5_command_PWM = 0; //unused
  m6_command_PWM = 0; //unused
  // --- Deadband/stop motors when CH5 low and throttle low ---
  // channel_3_pwm = throttle from RX (999..2000-ish)
  // channel_5_pwm = your switch (arm/kill style)
  if (channel_5_pwm < 1500 && channel_1_pwm <= 1015) {
    m1_command_PWM = 0;
    m2_command_PWM = 0;
    m3_command_PWM = 0;
  }
...
// ---- DShot600 helper functions -----------------------------------------------

int scaleToDShot(float scaled) {
  //DESCRIPTION: Map a 0.0-1.0 normalized motor command to a DShot600 throttle value.
  // Returns 0 (motor stop) for scaled <= 0; otherwise maps linearly to DSHOT_MIN_THROTTLE..DSHOT_MAX_THROTTLE.
  if (scaled <= 0.0f) return 0;
  return constrain((int)(DSHOT_MIN_THROTTLE + scaled * (DSHOT_MAX_THROTTLE - DSHOT_MIN_THROTTLE)),
                   DSHOT_MIN_THROTTLE, DSHOT_MAX_THROTTLE);
}

uint16_t dshotBuildFrame(uint16_t throttle, bool telemetry) {
  //DESCRIPTION: Build a 16-bit DShot frame from a throttle value and telemetry flag.
  // Frame layout: [11-bit throttle | 1-bit telemetry | 4-bit CRC], MSB first.
  // CRC = XOR of the three 4-bit nibbles of the 12-bit payload.
  // Throttle is clamped to the valid 11-bit range (0-2047).
  throttle = constrain(throttle, 0, 2047);
  uint16_t payload = (throttle << 1) | (telemetry ? 1 : 0);
  uint16_t crc = (payload ^ (payload >> 4) ^ (payload >> 8)) & 0x0F;
  return (payload << 4) | crc;
}

void dshotSendFrame(uint8_t pin, uint16_t frame) {
  //DESCRIPTION: Bit-bang a 16-bit DShot600 frame on the given pin (MSB first).
  // Called from within a noInterrupts() block in commandMotors().
  for (int i = 15; i >= 0; i--) {
    if ((frame >> i) & 0x1) {
      digitalWriteFast(pin, HIGH);
      delayNanoseconds(DSHOT600_T1H_NS);
      digitalWriteFast(pin, LOW);
      delayNanoseconds(DSHOT600_T1L_NS);
    } else {
      digitalWriteFast(pin, HIGH);
      delayNanoseconds(DSHOT600_T0H_NS);
      digitalWriteFast(pin, LOW);
      delayNanoseconds(DSHOT600_T0L_NS);
    }
  }
}
...
void commandMotors() {
  //DESCRIPTION: Send DShot600 frames to motor pins M1/M2/M3 (pins 11/12/14)
  /*
   * Builds a 16-bit DShot600 frame for each motor from its mX_command_PWM value
   * (0 = motor stop, DSHOT_MIN_THROTTLE..DSHOT_MAX_THROTTLE = spinning), then
   * bit-bangs the frame onto the corresponding output pin using precise nanosecond
   * delays available on the Teensy 4.0 (i.MX RT1062). No telemetry is requested.
   * Interrupts are disabled for the entire 3-motor transmission (~80 µs) to ensure
   * accurate bit timing across all three frames.
   */
  uint16_t frame1 = dshotBuildFrame((uint16_t)m1_command_PWM, false);
  uint16_t frame2 = dshotBuildFrame((uint16_t)m2_command_PWM, false);
  uint16_t frame3 = dshotBuildFrame((uint16_t)m3_command_PWM, false);
  noInterrupts();
  dshotSendFrame(m1Pin, frame1);
  dshotSendFrame(m2Pin, frame2);
  dshotSendFrame(m3Pin, frame3);
  interrupts();
}
...
void throttleCut() {
  ...
  /*
   * Monitors the state of radio command channel_5_pwm and directly sets the mX_command_PWM values to 0
   * (DShot stop command) if channel 5 is high. This is the last function called before commandMotors()
   * so that the last thing checked is if the user is giving permission to command the motors to anything
   * other than minimum value. Safety first. 
   */
  if (channel_5_pwm > 1500) {
    m1_command_PWM = 0; //DShot stop
    m2_command_PWM = 0;
    m3_command_PWM = 0;
    m4_command_PWM = 0;
    m5_command_PWM = 0;
